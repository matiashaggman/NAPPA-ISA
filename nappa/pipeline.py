...
def read_and_process_features(acc_path, gyro_path, time_zone, cfg=None):
    """
    ...
    Args:
        ...
        time_zone (str): Time zone of the place of recording
        with_classifier_features (bool): Determine whether to include only the 5 features (True) or all 11 features (False)
    Returns:
    ...
    # Sensor clock is in UTC+0. Convert timestamps to <location> time.
    accTime  = pd.to_datetime(accData[timeColName],  unit=unit, utc=True).dt.tz_convert(time_zone).dt.tz_localize(None)
    gyroTime = pd.to_datetime(gyroData[timeColName], unit=unit, utc=True).dt.tz_convert(time_zone).dt.tz_localize(None)
