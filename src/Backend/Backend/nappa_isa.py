...
def generate_pages(recording, tmp_dir, wear_idx, settings, status_callback=None):
    """
    ...
    settings (dict): Dictionary of settings for generating the report, including plot types and DPI settings.
    ...
    # MAIN PAGE
    pdf.add_page()
    main_fig, main_axes = make_main_fig(recording, wear_idx, settings, isMainPage=True)
    main_fig_path = os.path.join(tmp_dir, "main_fig.png")
    main_fig.savefig(main_fig_path, dpi=settings['report']['quality']['dpi'])
    plt.close(main_fig)

    # Template background
    pdf.image("templates/nappa_report_template.png", x=0, y=0, w=210, h=297)
    pdf.image(main_fig_path, x=6, y=35, w=200, h=133.333)
    ...
    if settings['visualization']['main_page']['sleep_statistics']:
        pdf.text(25, 254+10, f'Recording time: {str(sleep_statistics["total_time"])[:-3]}')
        pdf.text(25, 259+10, f'Total sleep time: {str(sleep_statistics["total_sleep"])[:-3]}')
        pdf.text(25, 264+10, f'Nonwear time: {str(sleep_statistics["nonwear_time"])[:-3]}')
    ...
    if isinstance(sleep_statistics["total_time"], timedelta) and sleep_statistics["total_time"].days >= 1:
        ...
        if settings['visualization']['main_page']['summaryfig'] == 'bar':
            ...
            plt.savefig(dist_fig_path, dpi=settings['report']['quality']['dpi'])
            plt.close()
            pdf.image(dist_fig_path, x=13, y=175, w=185, h=95)
        elif settings['visualization']['main_page']['summaryfig'] == 'violin':
            make_violin_fig(recording, wear_idx, settings)
            plt.savefig(dist_fig_path, dpi=settings['report']['quality']['dpi'])
            plt.close()
            pdf.image(dist_fig_path, x=10, y=175, w=185, h=95)

        if not settings['report']['output_formats']['figures']:
            os.remove(dist_fig_path)

    # SUBSEQUENT PAGES
    if settings['report']['layout']['multipage']:
        if settings['report']['layout']['auto_page_generation']:
            wear_blocks = detect_wear_blocks(wear_idx)
        else:
            wear_blocks = settings['data']['sleep_periods']  # Provided by user
        ...
        for i, (period_start, period_end) in enumerate(wear_blocks):
            ...
            pdf.add_page()
            sub_main_fig, _ = make_main_fig(sleep_period, sub_wear_idx, settings, isMainPage=False)
            sub_main_fig_path = os.path.join(tmp_dir, f"main_fig_{i}.png")
            sub_main_fig.savefig(sub_main_fig_path, dpi=settings['report']['quality']['dpi'])
            ...
            if settings['visualization']['subsequent_pages']['donut']:
                ...
                donut_fig.savefig(donut_fig_path, dpi=settings['report']['quality']['dpi'])
                plt.close(donut_fig)
            ...
            if settings['visualization']['subsequent_pages']['sleep_statistics']:
                pdf.text(110, 175, f'Recording time: {str(sp_statistics["total_time"])[:-3]}')
                pdf.text(110, 180, f'Nonwear time: {str(sp_statistics["nonwear_time"])[:-3]}')
                pdf.text(110, 185, f'Total sleep time: {str(sp_statistics["total_sleep"])[:-3]}')
               #pdf.text(110, 189, f'Number of awakenings during sleep: {str(sp_statistics["awakenings"])}')

    return pdf


def nappa_analysis(recording, wear_idx, output_file, tempfolder, settings, status_callback=None):
    """
    ...
    settings (dict, optional): A dictionary of settings to customize the analysis.
    ...
    2. Sets up the classifier based on the provided settings.
    ...
    7. Generates a PDF report and/or CSV output based on the provided settings.
    ...
    [sdt, lowerlim, upperlim] = caltrendwithCI(y[:, 1:])
...
async def startup_info(payload: dict):
    logger.info(f"User connected with payload: {payload}")
    ip = payload.get("IP")
    if ip:
        location = get_ip_location(ip)
        if location:
            logger.info(f"User's location: {location}")
        else:
            logger.info("Could not retrieve location for IP: {ip}")
    else:
        logger.info("No IP address provided in payload.")
...
async def import_recording(
    ...
    """
    Imports a zip archive containing sleep recording data.
    Parameters:
    zip_archive (UploadFile): The uploaded zip archive containing the sleep recording data.
    Returns:
    str: The path to the extracted zip file.
    """
    try:
        tempfolder = tempfile.mkdtemp()
        logger.info(f"Temporary folder created: {tempfolder}")
