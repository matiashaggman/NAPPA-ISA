...
def caltrendwithCI(probability_signal):
    ...
    Parameters:
        probability_signal (numpy.ndarray): A 2D array where each row represents
                                            probability distributions over three classes.
    
    Returns:
        weightedavg (numpy.ndarray): Weighted average trend estimate.
        lowerlim (numpy.ndarray): Lower confidence interval.
        upperlim (numpy.ndarray): Upper confidence interval.
    """
    
    nClasses = 3
    classes = np.array([1, 2, 3])

    # Create a copy of probability_signal to avoid modifying the input array
    prob_signal = np.copy(probability_signal)

    # Compute the weighted average using optimized matrix multiplication
    weights = np.array([0.5, 2, 3.5])
    weightedavg = np.sum(weights * prob_signal, axis=1)

    # Preallocate arrays for confidence limits
    lowerlim = np.zeros(weightedavg.shape)
    upperlim = np.zeros(weightedavg.shape)

    # Compute modal class
    modalC = np.round(weightedavg).astype(int)

    # Interpolate probability of estimated class
    interpprob = np.array([np.interp(weightedavg[i], classes, prob_signal[i]) for i in range(prob_signal.shape[0])])

    # Adjust probability of modal class
    prob_signal[np.arange(prob_signal.shape[0]), modalC - 1] -= interpprob

    # Compute confidence intervals
    pLowers = np.where(
        modalC == nClasses,
        np.sum(classes[:2] * prob_signal[:, :2], axis=1),
        np.where(
            modalC == 1,
            classes[0] * prob_signal[:, 0],
            np.where(
                modalC >= weightedavg,
                classes[0] * prob_signal[:, 0],
                np.sum(classes[:2] * prob_signal[:, :2], axis=1)
    ...
    pUppers = np.where(
        modalC == nClasses,
        classes[0] * prob_signal[:, nClasses - 1],
        np.where(
            modalC == 1,
            np.sum(classes[:2] * prob_signal[:, 1:3], axis=1),
            np.where(
                modalC >= weightedavg,
                np.sum(classes[:2] * prob_signal[:, 1:3], axis=1),
                classes[0] * prob_signal[:, 2]
            )
        )
    )

    lowerlim = weightedavg - np.abs(pLowers)
    upperlim = weightedavg + np.abs(pUppers)
    lowerlim = np.clip(lowerlim, 0.5, None)
    upperlim = np.clip(upperlim, None, nClasses + 0.5)
...
def plot_time_series(labels, predicted_probabilities,
                        ...
                        titles=('Hypnogram', 'Sleep Depth Trend')):
...
def plot_confusion_matrix(cm, class_names, size=(7,6), with_n=True, title='Confusion matrix'):
    ...
    if with_n:
        labels = np.asarray([f"{percent:.0%}\nn = {count}" for count,
                            percent in zip(cm.flatten(),
                            cm_percentages.flatten())]).reshape(cm.shape)
    else:
        labels = np.asarray([f"{percent:.0%}" for count,
                    percent in zip(cm.flatten(),
                    cm_percentages.flatten())]).reshape(cm.shape)   
