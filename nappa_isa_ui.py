...
class NappaMainWindow(QMainWindow, Ui_NappaDialog):
    ...
    def importRecording(self):
        ...
        self.worker = Worker(input_file=input_file, output_file=None,
                             options=self.options, call_type='import')  
    ...
    def initOptions(self):
        ...
        self.respirationSummaryBox.setChecked(self.options['plots']['respiration'])
        ...
        self.respirationSubsqBox.setChecked(self.options['plots']['subsequent_respiration'])
    ...
    def refreshOptions(self):
        ...
        self.options['plots']['respiration'] = self.respirationSummaryBox.isChecked()
        ...
        self.options['plots']['subsequent_respiration'] = self.respirationSubsqBox.isChecked()
    ...
    def selectPeriods(self):
        time_date_grid_window = TimeDateGridWindow(self.sleepPeriods)
    ...
    def onImportComplete(self, sleepRecording):
        if sleepRecording is not None:
            ...
            self.sleepPeriods = detect_wear_blocks(wear_idx)
            ...
            self.sleepPeriodsLabel.setText(f'{len(self.sleepPeriods)}')
            self.selectPeriodsButton.setEnabled(True)
